    ...
    <div className="space-y-4" id="saved-tab-container">
      ...
      <div className="pb-1 border-b border-[#D7E0EA] animate-fade-in-up" id="saved-header">
        <div className="flex items-center justify-between gap-2">
          <h2 className="text-xl font-extrabold text-[#0A1B4A]" id="saved-header-title">
            {t.savedLists}{' '}
            <span className="text-[#1B5BFF]">({favoritesLoading ? '…' : savedBusinesses.length})</span>
          </h2>
          <button
            type="button"
      ...
      {favoritesLoading && (
        <div className="flex items-center justify-center gap-2 py-10 text-gray-500 text-xs">
          <Loader2 className="w-4 h-4 animate-spin text-[#1B5BFF]" />
      ...
      {!favoritesLoading && !favoritesError && savedBusinesses.length === 0 && (
        <div className="text-center py-16 px-6 rounded-3xl bg-white border border-dashed border-[#D7E0EA]" id="saved-empty-state">
          ...
          <button
            ...
            className="mt-6 px-4 py-2 bg-[#1B5BFF] hover:bg-opacity-95 text-white font-extrabold text-xs rounded-xl transition-all shadow-md active:scale-95 flex items-center gap-1 mx-auto"
            id="saved-btn-browse"
          >
      ...
      {!favoritesLoading && savedBusinesses.length > 0 && (
        <div className="space-y-3" id="saved-grid-list">
          {savedBusinesses.map((biz) => (
            <div
              key={biz.id}
              className="p-3 rounded-2xl bg-white border border-[#D7E0EA] hover:border-[#1B5BFF]/30 transition-all relative flex gap-3.5 animate-fade-in-up card-hover"
            ...
            >
              ...
              >
                <div className="flex items-center gap-1.5 flex-wrap">
                  <span className="text-[8px] tracking-wider uppercase font-extrabold px-1.5 py-0.5 rounded bg-[#EEF2F6] text-[#1B5BFF] border border-[#D7E0EA]/80">
                    {textEn(biz.subcategory)}
                  </span>
                </div>
                <h3 className="text-xs font-black text-[#0A1B4A] hover:text-[#1B5BFF] truncate mt-1 leading-snug">
                  {biz.name}
                </h3>
              ...
              <div className="flex flex-col justify-between items-end" id={`saved-actions-${biz.id}`}>
                ...
                <div className="flex items-center gap-1.5 pb-0.5">
                  <Star className="w-3 h-3 text-[#1B5BFF] fill-[#1B5BFF]" />
                  <span className="text-[10px] font-black text-[#1B5BFF]">{biz.rating}</span>
                </div>
              </div>
            </div>
          ))}
