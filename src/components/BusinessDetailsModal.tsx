...
export const BusinessDetailsModal: React.FC<BusinessDetailsModalProps> = ({ business, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fade-in" id="details-modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="relative w-full max-w-2xl h-[90vh] overflow-y-auto rounded-3xl bg-[#EEF2F6] border border-[#D7E0EA] text-[#0B2545] scrollbar-thin" id="details-modal-container">
        ...
        <div className="absolute top-4 right-4 z-10 flex gap-2" id="details-floating-controls">
          <button
            ...
            className={`p-2.5 rounded-full backdrop-blur-md border border-[#D7E0EA] transition-colors ${
              isFav ? 'bg-[#00A859] text-white hover:bg-opacity-95' : 'bg-white/90 text-[#0B2545] hover:bg-white'
            }`}
            title={t.saved}
            id="details-btn-fav"
          >
          ...
          <button
            onClick={onClose}
            className="p-2.5 rounded-full backdrop-blur-md bg-black/50 hover:bg-black/80 text-[#00A859] border border-[#D7E0EA] transition-colors"
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            ...
            <div className="flex-1 min-w-0 md:mb-2">
              <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                <span className="px-2.5 py-0.5 rounded-full text-[10px] uppercase font-bold tracking-wider bg-[#EEF2F6] text-[#00A859] border border-[#D7E0EA]">
              ...
              <h1 className="text-2xl md:text-3xl font-extrabold text-[#0B2545] tracking-tight" id="details-biz-title">
                {business.name}
              </h1>
            ...
            <div className="flex flex-col items-center bg-[#EEF2F6] border border-[#D7E0EA] p-2.5 rounded-2xl w-24">
              <span className="text-xl font-black text-[#00A859] flex items-center gap-1">
        ...
        <div className="px-6 pb-24 grid grid-cols-1 md:grid-cols-3 gap-6" id="details-grid-body">
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            <div className="p-5 rounded-2xl bg-white border border-[#D7E0EA]">
              <h3 className="text-xs font-bold uppercase tracking-widest text-[#00A859] mb-3">
              ...
              <div className="mt-5 pt-4 border-t border-[#D7E0EA]/60 flex items-center gap-3 text-xs text-[#0B2545]/90" id="details-working-hours">
                <Clock className="w-4 h-4 text-[#00A859] shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <strong className="text-[#0B2545]">{t.workingHours}:</strong>
                    {isOpen !== null && (
                  ...
                  <span className="block mt-0.5 text-[#0B2545]/80">{textEn(business.workingHours)}</span>
                </div>
              </div>
            </div>

            {/* ── Rate This Business (interactive 5-star + POST /api/reviews) ── */}
            <div className="p-5 rounded-2xl bg-white border border-[#00A859]/25 space-y-4" id="details-rate-business">
              <div className="flex items-center justify-between gap-2">
                <h3 className="text-sm font-extrabold tracking-wider text-[#00A859]">
              ...
              <form onSubmit={handleCreateReview} className="space-y-4">
                        ...
                        >
                          <Star
                            className={`w-8 h-8 transition-colors ${
                              active ? 'text-[#00A859] fill-[#00A859]' : 'text-gray-700 hover:text-gray-500'
                            }`}
                          />
                        </button>
                      );
                    })}
                    <span className="ml-2 text-sm font-extrabold text-[#00A859]">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full p-3 rounded-xl bg-[#EEF2F6] border border-[#D7E0EA] focus:border-[#00A859] text-xs outline-none text-[#0B2545] transition-all disabled:opacity-50 resize-none"
                  />
                </div>
                ...
                <button
                  type="submit"
                  disabled={!currentUser || isSubmittingReview}
                  className="w-full py-3 rounded-xl bg-[#00A859] hover:bg-opacity-95 text-white font-extrabold text-xs transition-all shadow-md disabled:opacity-40 flex items-center justify-center gap-2"
            ...
            <div className="p-5 rounded-2xl bg-white border border-[#D7E0EA] space-y-4">
              <div className="flex items-center justify-between pb-2 border-b border-[#D7E0EA]">
                <h3 className="text-sm font-bold tracking-wider text-[#00A859]">
                  {t.reviews}
                </h3>
                  ...
                  businessReviews.map((rev) => (
                    <div key={rev.id} className="p-3.5 rounded-xl bg-[#EEF2F6] border border-[#D7E0EA]/40 space-y-2">
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="text-xs font-bold text-[#0B2545]">{rev.userName}</span>
                      ...
                      {rev.ownerReply ? (
                        <div className="mt-2 p-2.5 rounded-lg bg-[#EEF2F6] border border-[#D7E0EA]/60">
                          <p className="text-[10px] font-bold text-[#00A859] mb-1">
                        ...
                        <div className="mt-2 space-y-1.5">
                          ...
                          <button
                            ...
                            className="px-3 py-1.5 rounded-lg text-[10px] font-bold bg-[#00A859]/15 text-[#00A859] border border-[#00A859]/30 hover:bg-[#00A859]/25 disabled:opacity-50"
                          >
          ...
          <div className="space-y-6">
            ...
            <div className="p-5 rounded-2xl bg-white border border-[#D7E0EA] space-y-3" id="details-actions">
              <h3 className="text-xs font-bold uppercase tracking-widest text-[#00A859] mb-1">
                {t.contactBusiness}
              </h3>
              ...
              <button
                ...
                className="w-full flex items-center justify-between p-3.5 rounded-xl bg-[#00A859] hover:bg-[#008C4A] transition-all border border-[#008C4A] text-white group"
                id="action-btn-call"
              >
              ...
              >
                <span className="flex items-center gap-3 text-sm font-bold text-[#0B2545]">
                  <MessageSquare className="w-4 h-4 text-white" />
                  {t.openWhatsapp}
                </span>
                <ChevronRight className="w-4 h-4 text-white/80 group-hover:text-[#0B2545]" />
              </button>
              ...
              {business.website && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="w-full flex items-center justify-between p-3.5 rounded-xl bg-[#00A859]/15 hover:bg-[#00A859]/25 transition-all border border-[#00A859]/40 group"
                  id="action-link-website"
                >
                  <span className="flex items-center gap-3 text-sm font-bold text-[#00A859]">
                    <Globe className="w-4 h-4 text-[#00A859]" />
                    {t.openWebsite}
                  </span>
                  <ChevronRight className="w-4 h-4 text-[#00A859]/70 group-hover:text-[#00A859]" />
                </a>
              )}
            </div>
            ...
            <div className="p-5 rounded-2xl bg-white border border-[#D7E0EA] space-y-3">
              <h4 className="text-xs font-bold uppercase tracking-widest text-[#00A859]">
